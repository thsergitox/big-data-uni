...
readonly OTHER_PROJECT="hadoop2-lab"

if [[ -n "$(docker ps --quiet --filter "label=com.docker.compose.project=$OTHER_PROJECT")" ]]; then
  printf 'ERROR: Hadoop 2 está activo y usa los mismos puertos. Deténlo primero.\n' >&2
  exit 1
fi
...
printf '\nHadoop 3 está activo:\n'
printf '  HDFS NameNode:  http://localhost:9870\n'
