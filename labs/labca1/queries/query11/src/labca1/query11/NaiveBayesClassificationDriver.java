...
import labca1.common.GaussianNaiveBayesModel;
...
public final class NaiveBayesClassificationDriver {
  private NaiveBayesClassificationDriver() {}
  ...
  public static final class ModelReducer extends MapReduceBase
    ...
    public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter)
      ...
      GaussianNaiveBayesModel model = GaussianNaiveBayesModel.fit(features, labels);
      output.collect(new Text("MODEL"), new Text("GAUSSIAN_NAIVE_BAYES"));
      output.collect(new Text("TRAINING"), new Text(String.format(Locale.ROOT,
          "samples=%d\tthreshold=%.4f", training.size(), threshold)));
      emitMetrics(examples, threshold, model, output);
    }
  }

  private static void emitMetrics(List<Example> examples, double threshold,
      GaussianNaiveBayesModel model, OutputCollector<Text, Text> output) throws IOException {
  ...
  public static void main(String[] args) throws IOException {
    if (args.length != 2) throw new IllegalArgumentException("Uso: NaiveBayesClassificationDriver <input> <output>");
    JobConf job = new JobConf(NaiveBayesClassificationDriver.class);
    job.setJobName("labca1-query11-naive-bayes");
