...
public final class LinearRegressionDriver {
  private static final double LAMBDA = 0.0;

  private LinearRegressionDriver() {}
  ...
  private static String modelDescription(LinearRegressionModel model) {
    StringBuilder result = new StringBuilder(String.format(Locale.ROOT,
        "LINEAR_REGRESSION\tlambda=%.4f\tintercept=%.10f\tcoefficients=",
  ...
  public static void main(String[] args) throws IOException {
    if (args.length != 2) throw new IllegalArgumentException("Uso: LinearRegressionDriver <input> <output>");
    JobConf job = new JobConf(LinearRegressionDriver.class);
    job.setJobName("labca1-query13-linear-regression");
