...
public final class OccupancyByDepartmentDriver {
  private OccupancyByDepartmentDriver() {}
  ...
  public static final class OccupancyMapper extends MapReduceBase
    ...
    public void map(
      ...
      if (record.isOverall()) {
        output.collect(
            new Text(record.year() + ";" + record.department()),
  ...
  public static final class OccupancyReducer extends MapReduceBase
    ...
    public void reduce(
      ...
      while (values.hasNext()) {
        ...
        sum += Double.parseDouble(partial[0]);
        count += Long.parseLong(partial[1]);
      }
      output.collect(
          key, new Text(String.format(Locale.ROOT, "mean_tnoh=%.4f", sum / count)));
    }
  }

  public static void main(String[] args) throws IOException {
    if (args.length != 2) {
      throw new IllegalArgumentException("Uso: OccupancyByDepartmentDriver <input> <output>");
    }
    JobConf job = new JobConf(OccupancyByDepartmentDriver.class);
    job.setJobName("labca1-query02-occupancy-by-department");
